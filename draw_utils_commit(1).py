...
target_rgb = (255/255, 160/255, 122/255)
height = 10 # 10 * 10
width = 9 
...
class Render():
    def __init__(self, screen_width, screen_height, unit, start_point, data_path, log_path):
        ...
        self.draw_initial(data_path, type_="target")
        self.update_movement(start_point, 0)

    def draw_initial(self, data_path, type_, epi):
        if type_ == "box":
            ...
            position_data = pd.read_csv(os.path.join(data_path, "factory_order_train.csv"))
            box_data = pd.read_csv(os.path.join(data_path, "box.csv"))
            targetalpha = box_data["item"]
            targetrow = box_data["row"]
            targetcol = box_data["col"]
            for target in position_data[epi]
                self.targets.append((targetrow[targetalpha.index(target)], targetcol[targetalpha.index(target)]))
            color = target_rgb
        else:
            # TODO error
            pass
